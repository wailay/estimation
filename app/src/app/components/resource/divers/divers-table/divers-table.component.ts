...
export class DiversTableComponent implements OnChanges {
    table: Tabulator;
    @Input() data: any[] = [];
    ...
    private drawTable(): void {
        this.table = new Tabulator('#divers-table', {
            data: this.data,
    ...
    private openTypeForm(row): void {
        ...
        modal.afterClose.subscribe((type) => {
            if (!type) return;
            this.resourceService.addType(type.code, 'D').then((res) => {
                if (res.status === 'error') {
                    console.log('error');
                    return;
                } else {
    ...
    private openResourceForm(row): void {
        ...
        modal.afterClose.subscribe((resource) => {
            if (!resource) return;
            this.resourceService.addResource(resource, parentId, 'D').then((res) => {
                console.log(res);
