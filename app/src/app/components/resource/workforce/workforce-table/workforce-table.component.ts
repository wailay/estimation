...
export class WorkforceTableComponent implements OnChanges {
    table: Tabulator;
    @Input() data: any[] = [];
    ...
    private openTypeForm(row): void {
        ...
        modal.afterClose.subscribe((type) => {
            if (!type) return;
            this.resourceService.addType(type.code, 'W').then((res) => {
                if (res.status === 'error') {
                    console.log('error');
                    return;
                } else {
    ...
    private openResourceForm(row): void {
        ...
        modal.afterClose.subscribe((resource) => {
            if (!resource) return;
            this.resourceService.addResource(resource, parentId, 'W').then((res) => {
                console.log(res);
